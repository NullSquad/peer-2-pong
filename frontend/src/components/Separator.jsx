...
export const Separator = ({ children }) => {
  const { user } = useAuth();
  return (
    <header className="relative mb-10 pt-10">
      {/* Elemento principal (siempre encima) */}
      <div
        className="absolute z-20 bg-yellow-500 text-white sm:mt-15 py-1 px-5 sm:px-10 sm:ml-[-1%] ml-[-2%] w-auto sm:w-[35%] xl:w-[20%] min-w-[150px] flex justify-end
      ...
      <div
        className="relative z-10 bg-yellow-500 text-white py-1.5 px-5 sm:px-10 sm:ml-[-1%] ml-[-2%] w-auto sm:w-[60%] xl:w-[40%] min-w-[200px] flex justify-between 
      ...
      ></div>
      <div
        className="relative z-0 bg-yellow-500 text-white py-1.5 px-5 sm:px-10 sm:ml-[-1%] ml-[-2%] w-auto sm:w-[45%] xl:w-[30%] min-w-[150px] flex justify-between 
		items-center border-t-[8px] border-t-yellow-50 border-b-[8px] border-b-yellow-700 border-r-[4px] border-r-black shadow-md transform -skew-x-[8deg]"
      ></div>
    </header>
  );
