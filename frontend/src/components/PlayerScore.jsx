...
export const ButtonScore = ({ Sign, Operation, color, effect }) => {
  ...
  return (
    <>
      <button
        onClick={Operation}
        className={`${bgColorClass} ${shadowClass} border-2 border-black w-8 h-6 sm:w-9 sm:h-7 mr-1 rounded-md text-white text-sm sm:text-xl font-bold font-lilita-one active:translate-y-[1px] flex items-center justify-center -skew-x-[5deg]`}
      >
        <SignScore Sign={Sign} />
      </button>
    </>
