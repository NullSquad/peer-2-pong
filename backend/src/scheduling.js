...
agenda.define("start competition", (job) => {
  ...
  agenda.every(
    competition.settings.frequency,
...
function get_competition({ competitionId }) {
  ...
  const competitions = db
    ...
    .then((data) => data.findOne(query).then(data));
...
function schedule_competition({ competitionId }) {
  ...
  agenda.schedule(competition.startDate, "start competition", {
    competitionId,
  });
  return;
}
...
function check_outdate_matches(competition) {
  ...
  for (match in matches) {
    if (match.status != "finished") tie_match(match);
  }
}
...
function create_league_matches(competition) {
  ...
  for (rank in Object.values(leagues)) {
    competition_matches = create_new_matches(
      players.slice(first_player, rank),
      competition.quant,
    );
...
function create_new_matches(players) {
  ...
  if (matches[-1].length == 1) matches[-1].push(freepoints_user);
  return matches;
}
